		...
		if (isOk) {
			...
			const products = await this.page.$$eval(
				...
				(anchorEls, sel) => {
					...
					// ! gets closer elements by text or css class name
					const findNearWithText = (el, textPattern = "", cssClass = "", maxUp = 5) => {
						...
						.map((el) => {
							const title = sel.title
								...
								: findNearWithText(
										el,
										"",
										'h1,h2,h3,h4,h5,h5,span,p,[class*="title"],[class*="name"],[class*="product"]'
									);
