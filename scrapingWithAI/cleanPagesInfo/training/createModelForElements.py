...
from sklearn.multioutput import MultiOutputClassifier       # validation is 1 or 0, and type
...
# accuracy per thing
print("is_valid:", classification_report(y_test[:, 0], y_pred[:, 0]))
print("type:", classification_report(y_test[:, 1], y_pred[:, 1]))
...
def complete_output(y_all_pred):
    ...
    complete.to_csv("./trainingResults/elements/completeElementsClassification.csv", index=False)
...
def filter_fine(y_all_pred):
    ...
    filtered_df.to_csv("./response/elements/onlyFineElements.csv", index=False)
