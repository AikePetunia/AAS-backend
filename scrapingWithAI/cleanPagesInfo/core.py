...
armyTech: new SiteConfig({
    ...
    categories: [                                   <- createModelForPaths.py
      "/405-hardware",
      "/436-perifericos",
      "/398-gaming-house",
      "/431-monitores-y-tvs",
      "/373-conectividad",
      "/389-energia",                               <- A good way to see if the paths were correctly selected, it's getting the elements to similar paths, and if they are slightly different, there are correct. This could add accuaracy
    ],
    selectors: {                                    <- From the getClassesFromPath && createModelForElements.py
...
# 2. Run pathsClassification.py
# 3. Extract at least 1 path from the output. This could be given one classified path, or manual.
# 4. Run classesFromPaths.mjs
# 5. Run elementsClassification.py
# 6. Run core.py
import json
    ...
    for elem in elements_:
        ...
        for key in valid_types:
            if key in elem:
                ...
                # some elements are finishing with the .
...
with open("response/scrapeFinalInfo.json", "w", encoding="utf-8") as f:
