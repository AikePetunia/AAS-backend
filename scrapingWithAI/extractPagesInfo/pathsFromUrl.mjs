...
async function setPathsDataset(allResults) {
	...
	await fs.writeFile("./scrapingWithAI/playwright/resultPaths/pathsDataset.csv", csvContent);
}

async function setPathsToClassify(allResults) {
	await fs.writeFile(
		"./scrapingWithAI/playwright/resultPaths/pathsToClassify.json",
...
async function setDomainsWithoutPaths() {
	if (stats.failedPagesNames.size > 0) {
		await fs.writeFile(
			"./scrapingWithAI/playwright/resultPaths/domainsWithoutPaths.json",
